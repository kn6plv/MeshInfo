...
async function readNode(ip) {
    Log('readNode', ip);
    return new Promise(async resolve => {
        try {
            const ac = new AbortController();
            setTimeout(() => ac.abort(), FETCH_TIMEOUT);
            const req = await fetch(`http://${ip}/cgi-bin/sysinfo.json?link_info=1&lqm=1`, { signal: ac.signal });
        ...
        try {
            const ac = new AbortController();
            setTimeout(() => ac.abort(), FETCH_TIMEOUT);
            const req = await fetch(`http://${ip}:8080/cgi-bin/sysinfo.json?link_info=1&lqm=1`, { signal: ac.signal });
            const v = await req.json();
            console.log(`${ip}: success`);
            resolve(v);
            return;
            ...
            Log('failed 8080', ip, e);
            resolve(null);
        }
    });
}

async function readNodeList(ip) {
    Log('readNodeList', ip);
    return new Promise(async resolve => {
        try {
            const ac = new AbortController();
            setTimeout(() => ac.abort(), FETCH_TIMEOUT);
            const req = await fetch(`http://${ip}:9090`, { signal: ac.signal });
            const v = await req.json();
            let nodes = {};
            v.routes.forEach(r => {
                if (r.destination.substr(0, 3) == "10." && r.genmask == 32 && r.networkInterface == "br-dtdlink") {
...
module.exports = {
    ...
    update: async function (oldjson) {
        ...
        const list = await readNodeList(ROOT);

        pending.push({ ip: ROOT, attempts: 0 });
        list.forEach(ip => {
            pending.push({ ip: ip, attempts: 0 });
        });
        pending.push({ ip: ROOT, attempts: 0 });
        ...
        async function crawl() {
            ...
            if (next) {
                const node = await readNode(next.ip);
        ...
        console.log(`*** Nodes: available ${list.length} found ${nodes.length}`);
